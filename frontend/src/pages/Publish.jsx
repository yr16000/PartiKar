...
// shadcn DateRangePicker (Calendar + Popover + Command)
...
// >>> NEW: Select import (pour Climatisation et cohérence UI)
import {
    Select,
    SelectTrigger,
    SelectContent,
...
export default function Publish() {
    const [form, setForm] = useState({
        ...
        boiteVitesse: "",
        nbPlaces: "",
        climatisation: false, // reste bool côté state/payload
        couleur: "",
        description: "",
    });

    // Helpers génériques pour les inputs classiques
    const onChange = (e) => {
        const { name, value } = e.target;
        setForm((prev) => ({ ...prev, [name]: value }));
    };

    // >>> Disponibilité (DateRangePicker) — même logique visuelle que Hero, sans heures
    const today = useMemo(() => {
        const d = new Date();
        d.setHours(0, 0, 0, 0);
        return d;
    ...
    const validate = () => {
        ...
        if (isNaN(Number(form.nbPlaces)) || Number(form.nbPlaces) <= 0) {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            voiture: {
                ...
                climatisation: !!form.climatisation, // bool propre
                ...
                <Card className="w-full max-w-3xl border border-border bg-card shadow-lg">
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            {/* Places / Clim (Select Oui/Non) */}
                            <div className="grid md:grid-cols-2 gap-6">
                                <div>
                                    <Label className="mb-2 block text-sm">Nombre de places *</Label>
                                    <Input name="nbPlaces" type="number" min={1} value={form.nbPlaces} onChange={onChange} required />
                                </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <Label className="mb-2 block text-sm">Ville *</Label>
                                    <Input name="ville" value={form.ville} onChange={onChange} placeholder="Paris" required />
                                </div>
                            </div>
                            ...
                            {/* Prix + Disponibilité (DateRangePicker façon Hero, sans heures) */}
                            <div className="grid md:grid-cols-2 gap-6">
                                <div>
